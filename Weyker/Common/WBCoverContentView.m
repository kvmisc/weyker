...
@implementation WBCoverContentView
...
- (void)prepareForView:(UIView *)inView viewport:(UIView *)viewport
{
  ...
  _coverView.status = WBCoverViewStatusUnknown;
  [_coverView removeAllAnimations];

  WBCoverView *coverView = _coverView;
  if ( !coverView ) {
    // 无 coverView，创建
    coverView = [[WBCoverView alloc] init];
