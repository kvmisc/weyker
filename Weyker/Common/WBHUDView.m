...
{
  if ( completion ) {
    if ( self.coverView.status==WBCoverViewStatusShowing ) {
      self.layer.opacity = 1.0;
    } else if ( self.coverView.status==WBCoverViewStatusHiding ) {
      self.layer.opacity = 0.0;
    }
  } else {
    if ( (self.coverView.status==WBCoverViewStatusShowing) || (self.coverView.status==WBCoverViewStatusHiding) ) {
