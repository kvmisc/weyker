...
- (void)setup
{
  ...
  _status = WBCoverViewStatusUnknown;
...
- (BOOL)isShowing
{
  return _status==WBCoverViewStatusShowing;
}
- (BOOL)isHiding
{
  return _status==WBCoverViewStatusHiding;
}
...
{
  WBLogDebug(@"CoverView", @"animation start: %@", anim);
}
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
  if ( anim==[self.layer animationForKey:@"ShowSelf"] ) {
    WBLogDebug(@"CoverView", @"show self stop: %@", anim);
    ...
    if ( flag ) {
      ...
      _status = WBCoverViewStatusShowFailed;
    }
    [self.layer removeAnimationForKey:@"ShowSelf"];
    return;
  }
  ...
  if ( anim==[_contentView.layer animationForKey:@"ShowContent"] ) {
    WBLogDebug(@"CoverView", @"show content stop: %@", anim);
    [_contentView updateStateFromAnimation:flag];
    if ( flag ) {
      // 已完成 ShowContent，置于 Presented 状态
      _status = WBCoverViewStatusPresented;
    } else {
  ...
  if ( anim==[_contentView.layer animationForKey:@"HideContent"] ) {
    WBLogDebug(@"CoverView", @"hide content stop: %@", anim);
    [_contentView updateStateFromAnimation:flag];
    if ( flag ) {
      ...
      if ( _needsSelfAnimation ) {
        ...
        WBLogDebug(@"CoverView", @"direct hide self");
      ...
      _status = WBCoverViewStatusHideFailed;
    }
    [_contentView.layer removeAnimationForKey:@"HideContent"];
    return;
  }
  ...
  if ( anim==[self.layer animationForKey:@"HideSelf"] ) {
    WBLogDebug(@"CoverView", @"hide self stop: %@", anim);
    [self updateStateFromAnimation:flag];
    if ( flag ) {
...
- (void)show:(BOOL)animated
{
  ...
  if ( animated ) {
    // 需要动画
    if ( _status==WBCoverViewStatusShowing ) {
      // 正在显示，不管
    } else if ( _status==WBCoverViewStatusShowFailed ) {
      // 显示失败，重启显示
      [self doShow:YES];
    } else if ( _status==WBCoverViewStatusPresented ) {
      // 已经显示，不管
    } else if ( _status==WBCoverViewStatusHiding ) {
      ...
      [self doShow:YES];
    } else if ( _status==WBCoverViewStatusHideFailed ) {
      // 隐藏失败，启动显示
      [self doShow:YES];
    } else {
      [self doShow:YES];
...
{
  _status = WBCoverViewStatusShowing;
  ...
  if ( animated ) {
    ...
    if ( _needsSelfAnimation ) {
      ...
      WBLogDebug(@"CoverView", @"direct show self");
      [self updateStateFromAnimation:YES];
      [self addShowContentAnimation];
    }
...
- (void)doHide:(BOOL)animated
{
  ...
  _status = WBCoverViewStatusHiding;
...
- (void)doComplete
{
  ...
  WBLogDebug(@"CoverView", @"hide done");
}
